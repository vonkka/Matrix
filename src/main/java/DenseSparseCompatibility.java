...
public class DenseSparseCompatibility {
    boolean compCombinations(String mat1, String mat2, String matRes) throws IOException {
        ...
        try {
            ...
            if (!d1.mul(d2).equals(s1.mul(s2)) ||
                    !s1.mul(s2).equals(d1.mul(d2)) ||
                    !d1.mul(d2).equals(d1.mul(s2)) ||
                    !s1.mul(s2).equals(d1.mul(s2)) ||
                    !d1.mul(s2).equals(s1.mul(d2)) ||
                    !d1.mul(s2).equals(s1.mul(s2)) ||
                    !s1.mul(d2).equals(d1.mul(s2)) ||
                    !s1.mul(d2).equals(d1.mul(d2)) ||
                    !s1.mul(d2).equals(s1.mul(s2))) return false;
            if (!dres.equals(sres) || !d1.mul(d2).equals(sres)) return false;
            return true;
        }
        catch (Exception e) {
